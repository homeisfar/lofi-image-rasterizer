...
public class DitherColor {
        ...
        */
        ...
        // DitherColor (BufferedImage origImage) {
    ...
    */
            ...
            private static final void parallelOrderedDither(ImageData img, int row, int i, Ditherable.Dither d) {
                ...
                for (int c = i; c < i + img.width; c++) {
                    ...
                    // ? BLACK : WHITE;
                    imgData[c] = findClosestColorFromPalette(img.reds[c] + (int) (palette.length * (bayer[bayerHorizontalOffset + bayerVerticalOffset] - .5)),
                                                           img.greens[c] + (int) (palette.length * (bayer[bayerHorizontalOffset + bayerVerticalOffset] - .5)),
                                                            img.blues[c] + (int) (palette.length * (bayer[bayerHorizontalOffset + bayerVerticalOffset] - .5)));
                }
            }
            ...
            private static final void parallelRandomDither(ImageData img, int i) {
                ...
                for (int c = i; c < i + img.width; c++) {
                    ...
                    // ? BLACK : WHITE;
                    imgData[c] = findClosestColorFromPalette(img.reds[c], img.greens[c], img.blues[c]);
                }
            }
            ...
            //         // IntStream.range(0, luminosityMatrixFast.length).parallel().forEach(i -> lumiTest(i, imgData));
            //         // IntStream.range(0, img.height).parallel().forEach(i -> lumiTest(i * img.width));
            //         IntStream.range(0, img.height).parallel().forEach(i -> parallelOrderedDither(i, i * img.width, Dither.SIMPLE));
            // }

            // private static final void simple2TEST(ImageData img) {
            //     for (int r = 0; r < img.height; r++) {
            //         int index = r * img.width;
            //         for (int c = 0; c < img.width; c++) {
            //             imgData[index] = findClosestColorFromPalette(img.reds[index], img.greens[index], img.blues[index]);
            //             index++;
            //         }
            //     }
            // }
            ...
            //             if (j + 1 < img.width) cloneMatrix[index + 1] += quantError * (7.0 / 16.0);
            //             if (i + 1 == img.height) continue;
            //             if (j > 0)                     cloneMatrix[index + img.width - 1] += quantError * (3.0 / 16.0);
            //             /* no if */                    cloneMatrix[index + img.width    ] += quantError * (5.0 / 16.0);
            //             if (j + 1 < img.width) cloneMatrix[index + img.width + 1] += quantError * (7.0 / 16.0);
            //         }
            //     }
            // }
